...
define(function (require) {
  ...
  var jsDelivrBase = "/third-party/cdn.jsdelivr.net/npm";

  var urlArgs = "__v__=" + +new Date();

  var topCode = [`export * from "echarts/src/echarts";`];
  var srcFolder = version.startsWith("5.0.0")
    ? "esm" // Only 5.0.0 has esm folder
    : isVersion5
    ? "lib"
    : "src";
  ...
  function resolveNpmDependencies(pkg, version) {
    return fetch(`${jsDelivrBase}/${pkg}@${version}/package.json`, {
      mode: "cors",
    })
  ...
  resolveNpmDependencies("echarts", version).then(startRollup);
  ...
  function startRollup() {
    rollup
      ...
      .then(function (result) {
        ...
        code = transformDev.transform(
          code,
          false,
          !BUILD_CONFIG.source ? "false" : "true"
        ).code;
